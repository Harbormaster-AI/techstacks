...
public class ${className}RestController extends $parentController {
    ...
    public ${className} update( @RequestBody(required=true) ${classObject.getUpdateCommandAlias()} command ) {
		...
		try {                        	        
			...
			// -----------------------------------------------
			entity = ${className}BusinessDelegate.get${className}Instance().update${className}(command);;
	    }
	    catch( Throwable exc ) {
    ...
    public void delete( @RequestBody(required=true) ${classObject.getDeleteCommandAlias()} command ) {                
    	try {
